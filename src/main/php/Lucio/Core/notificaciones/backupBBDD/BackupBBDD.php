<?php
namespace Lucio\Core\notificaciones\backupBBDD;

use Lucio\Core\utils\XTemplate;
...
class BackupBBDD{
	...
	function backupDatabaseTables($dbHost,$dbUsername,$dbPassword,$dbName, $filename,$tables = '*' ){
	    ...
	    //save file
		$nameFile ='db_Lucio_' . date("Y-m-d-H-i-s") .'.sql';
	...
	public function send(){
		...
		$this->setPathBackup(LucioConfig::getInstance()->getAppPathCore()."/backup/");

		$filename = 'db_lucio_' . date("Y-m-d-H-i-s") . '.zip';
		$backupFile = $this->getPathBackup().$filename;

		self::backupDatabaseTables(LucioConfig::getInstance()->getDbHost(),LucioConfig::getInstance()->getDbUser(),LucioConfig::getInstance()->getDbPassword(),LucioConfig::getInstance()->getDbName(),$filename);
		...
		$template->assign("titulo", "En el archivo adjunto se encuentra el BackUp de la BBDD cose_lucio realizado el ".date('d/m/Y H:i:s'));
		...
		LucioUtils::sendMail(LucioConfig::TEST_MAIL_TO,LucioConfig::TEST_MAIL_TO, $subjectMail, $mensaje, $attachs);
		unlink($backupFile);
